...
func initialize(ctx context.Context, client querier) *changefeedCache {
	...
	lock.Lock()
	if cache == nil {
		cache = newChangefeedCache(ctx, client)
		cache.waitForSync(ctx, apd.New(time.Now().UnixNano(), 0))
	}
	c := cache
	lock.Unlock()
	return c
...
type changefeedCache struct {
	...
	// TODO: can we use a chan struct{} here, close it to broadcast, and slap a new one into its place after?
	incoming *sync.Cond
...
func newChangefeedCache(ctx context.Context, client querier) *changefeedCache {
	...
	wg.Add(1)
	go func(ctx context.Context, client querier, events chan<- *event, cache *changefeedCache) {
		defer wg.Done()
		for {
			select {
			case <-ctx.Done():
			...
			highWaterMark := cache.highWaterMark // TODO: first time around, start 5min ago?
	...
	wg.Add(1)
	go func(ctx context.Context, cache *changefeedCache, events <-chan *event) {
		defer wg.Done()
		for {
			select {
			case evt := <-events:
...
func consumeChangefeed(ctx context.Context, client querier, timestamp *apd.Decimal, events chan<- *event) (terminal bool) {
	options := []string{"diff", "mvcc_timestamp", fmt.Sprintf("resolved='%s'", resolveInterval.String())}
	if timestamp != nil {
		options = append(options, fmt.Sprintf("cursor='%s'", timestamp.String()))
	}
...
func (c *changefeedCache) process(evt *event) {
	c.bufferLock.Lock()
	// find the index where we need to append this into the buffer
	i := sort.Search(len(c.buffer.items), func(i int) bool { return c.buffer.items[i].data.rawRev.Cmp(evt.rawRev) >= 0 })
	if i < len(c.buffer.items) && c.buffer.items[i].data.rawRev.Cmp(evt.rawRev) == 0 {
	...
	evtNode := &linkedListNode{data: evt}
	...
	for i := range c.buffer.items {
		items = append(items, c.buffer.items[i].data.rawRev.String())
	}

	if evt.eType == eventTypeProgressNotify {
		c.highWaterMark = evt.rawRev
		// move events up to the resolved timestamp into storage
		var marker *skipListNode
		for _, buffered := range c.buffer.items[:i+1] {
			if node := c.history.append(buffered.data); buffered.data.rawRev.Cmp(evt.rawRev) == 0 {
				marker = node
			}
		}
...
func (c *changefeedCache) waitForSync(ctx context.Context, timestamp *apd.Decimal) bool {
	...
	for {
		...
		if !eventResolved {
			...
			idx := sort.Search(len(c.buffer.items), func(i int) bool { return c.buffer.items[i].data.rawRev.Cmp(timestamp) >= 0 })
			eventBuffered = idx < len(c.buffer.items) && c.buffer.items[idx].data.rawRev.Cmp(timestamp) == 0
		}
...
func (c *changefeedCache) subscribeAfter(ctx context.Context, timestamp *apd.Decimal, events chan<- *event) {
	c.clients.Add(1)
	defer c.clients.Done()
	compacted := c.waitForSync(ctx, timestamp)

	c.bufferLock.RLock()
	if compacted || (c.history.heads[0] != nil && timestamp.Cmp(c.history.heads[0].value) < 0) {
		events <- &event{eType: eventTypeCompacted}
		return
	}
	...
	if len(c.buffer.items) == 0 || timestamp.Cmp(c.buffer.items[0].data.rawRev) < 0 {
		...
		bufferIndex := sort.Search(len(c.buffer.items), func(i int) bool { return c.buffer.items[i].data.rawRev.Cmp(timestamp) >= 0 })
	...
	for {
		...
		if timestampResolved || timestamp.Cmp(bufferStart.data.rawRev) <= 0 {
			timestampResolved = bufferStart.data.eType == eventTypeProgressNotify // stop checking once we know everything else will be true
			events <- bufferStart.data
		}
	}
}
...
func (l *skipList) append(data *event) *skipListNode {
	node := skipListNode{
...
func (l *skipList) find(value *apd.Decimal) *skipListNode {
	...
	for {
		...
		if next != nil && next.value.Cmp(value) <= 0 {
...
func (l *skipList) resetHead(newHead *skipListNode) {
	...
	for level := 0; level <= l.largestLevel; level++ {
		...
		for {
			next := head.next[level]
			if next != nil && next.value.Cmp(newHead.value) < 0 {
