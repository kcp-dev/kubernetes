...
func CreateCRDUsingRemovedAPI(storageClient generic.Client, etcdStoragePrefix string, betaCRD *apiextensionsv1beta1.CustomResourceDefinition, apiExtensionsClient clientset.Interface, dynamicClientSet dynamic.Interface) (*apiextensionsv1.CustomResourceDefinition, error) {
	crd, err := CreateCRDUsingRemovedAPIWatchUnsafe(storageClient, etcdStoragePrefix, betaCRD, apiExtensionsClient)
	if err != nil {
		return nil, err
	}
...
// with removed data.  Do not use this just because you don't want to update your test to use v1.  Only use this
// when it actually matters.
func CreateCRDUsingRemovedAPIWatchUnsafe(storageClient generic.Client, etcdStoragePrefix string, betaCRD *apiextensionsv1beta1.CustomResourceDefinition, apiExtensionsClient clientset.Interface) (*apiextensionsv1.CustomResourceDefinition, error) {
	...
	if succeeded, _, err := storageClient.Create(ctx, key, val, 0); err != nil {
		...
		return nil, errors.New("failed to create custom resource definition using raw storage client")
	}
