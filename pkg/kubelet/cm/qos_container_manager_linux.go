...
func (m *qosContainerManagerImpl) Start(getNodeAllocatable func() v1.ResourceList, activePods ActivePodsFunc) error {
	...
	if exists, why := cm.Exists(rootContainer); !exists {
		return fmt.Errorf("root container %v doesn't exist: %s", rootContainer, why)
	}
	...
	for qosClass, containerName := range qosClasses {
		...
		// check if it exists
		if exists, _ := cm.Exists(containerName); !exists {
